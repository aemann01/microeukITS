...
cd /home/user/mann
bcl2fastq -R /media/Synology_Data/Seq_Run_Data/LMAMR51_Mar_2019/ -o ITSfastq --use-bases-mask Y*,I*,I*,Y* --sample-sheet sampleSheet.csv
...
median=$(awk '{print $2}' readcount.txt | sort -n | awk ' { a[i++]=$1; } END { x=int((i+1)/2); if (x < (i+1)/2) print (a[x-1]+a[x])/2; else print a[x-1]; }')
